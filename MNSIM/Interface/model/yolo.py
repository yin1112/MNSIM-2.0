...
import numpy as np
from utils.voc2007.tools import loss
...
class myNet(nn.Module):
    def __init__(self, net  , device, input_size=None, num_classes=20, trainable=False, conf_thresh=0.01, nms_thresh=0.5, hr=False):
    ...
    def forward(self, x , MODE, target=None):


        prediction = self.model(x ,MODE)

        # print(prediction.shape)

        prediction = prediction.view(x.size(0), 1 + self.num_classes + 4, -1).permute(0, 2, 1)
